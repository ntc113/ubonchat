      ...
      attach = function () {
        appCache.addEventListener('updateready', function (e) {
          if (appCache.status == appCache.UPDATEREADY) {
            if (!declined) {
              safeConfirm({type: 'WEBOGRAM_UPDATED_RELOAD', message: 'A new version of UBon is downloaded. Launch it?'}, function (result) {
                if (result) {
...
(function initApplication () {
  ...
  ConfigStorage.get('current_layout', 'i18n_locale', function (params) {
    ...
    if (!locale) {
      locale = (window.navigator.userLanguage || window.navigator.language).toLowerCase();
