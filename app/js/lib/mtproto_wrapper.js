...
.factory('MtpApiManager', function (Storage, ErrorService, qSync, $q, socket) {
  ...
  function getContacts () {
    return Storage.get('friendlist').then(function (result) {
      return result || {};
    });
  }
  ...
  function mtpSetUserAuth (userAuth) {
    Storage.set({
      user_auth: userAuth
    });
  }
  ...
  function mtpLogOut () {
    /*ntc113*/
    socket.disconnect();
    Storage.remove('dc', 'user_auth', 'friendlist');
    baseDcID = false;   
  }
